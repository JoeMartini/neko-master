...
export const api = {
  ...
  createBackend: (backend: { name: string; url: string; token?: string; type?: 'clash' | 'surge' | 'conntrack' }) =>
    fetchJson<{ id: number; isActive?: boolean; message: string; agentToken?: string }>(`${API_BASE}/backends`, 'POST', backend),
    
  updateBackend: (id: number, backend: { name?: string; url?: string; token?: string; type?: 'clash' | 'surge' | 'conntrack'; enabled?: boolean; listening?: boolean }) =>
  ...
  testBackend: (url: string, token?: string, type?: 'clash' | 'surge' | 'conntrack') =>
